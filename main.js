...
function fetchData() {
  ...
  fetch(
    `http://rt.ex7.pl/get-data?sort_column=${sort_column}&sort_order=${sort_order}&filter=${filter}&page_size=${page_size}&page=${page}`,
    { method: "POST" }
  )
    .then(res => res.json())
    .then(data => {
    ...
    `;
      ...
      fetch("http://rt.ex7.pl/get-data?page_size=1000", { method: "POST" })
        ...
        //   var paramsString = `q=URLUtils.searchParams&page=${page}`;
